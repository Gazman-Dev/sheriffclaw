...
from python_openclaw.channels.telegram_secure_bot import TelegramSecureBotAdapter, TelegramSecureBotClient, TelegramSecureBotRunner
...
def _build_core(base: Path) -> tuple[GatewayCore, IdentityManager, SecretStore, ApprovalGate]:
    ...
    for uid in set(config.get("operators", [])):
        identities.allow_operator(int(uid))
    for uid in set(config.get("llm_users", [])):
    ...
    return core, identities, secrets, approval_gate
...
def build_runtime(base_dir: Path | None = None) -> OpenClawRuntime:
    ...
    core, identities, secrets, approval_gate = _build_core(base)
    ...
    secure_adapter = TelegramSecureBotAdapter(
        identities=identities,
        secrets=secrets,
        bot_client=TelegramSecureBotClient(gate_bot),
        approval_gate=approval_gate,
    )

    return OpenClawRuntime(
        llm_runner=TelegramLLMBotRunner(llm_adapter, llm_bot),
        secure_runner=TelegramSecureBotRunner(secure_adapter, gate_bot),
    )


def build_agent_runtime(base_dir: Path | None = None) -> TelegramLLMBotRunner:
    base = base_dir or Path.cwd()
    core, identities, _secrets, _approval_gate = _build_core(base)
    llm_bot = _build_bot("OPENCLAW_AGENT_TOKEN")
    llm_adapter = TelegramLLMBotAdapter(gateway=core, identities=identities, bot_client=TelegramLLMBotClient(llm_bot))
    return TelegramLLMBotRunner(llm_adapter, llm_bot)


def build_gate_runtime(base_dir: Path | None = None) -> TelegramSecureBotRunner:
    base = base_dir or Path.cwd()
    _core, identities, secrets, approval_gate = _build_core(base)
    gate_bot = _build_bot("OPENCLAW_GATE_TOKEN")
    secure_adapter = TelegramSecureBotAdapter(
        identities=identities,
        secrets=secrets,
    ...
    return TelegramSecureBotRunner(secure_adapter, gate_bot)

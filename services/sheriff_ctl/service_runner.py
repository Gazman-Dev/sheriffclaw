...
LLM_ORDER =["codex-mcp-host", "ai-tg-llm", "telegram-listener"]
    ...
    if service == "codex-mcp-host":
        ...
        if platform.system() == "Darwin":
            ...
            if user:
                if not _posix_user_exists(user):
                    raise RuntimeError(f"configured codex-mcp-host user does not exist: {user}")
                launcher = _darwin_ai_worker_launcher_path()
                if not launcher.exists():
                    raise RuntimeError(
                        f"missing macOS codex-mcp-host launcher: {launcher} (rerun installer)")
        ...
        if sandboxed is None:
            if _strict_sandbox_required():
                raise RuntimeError(
                    "strict sandbox enabled: codex-mcp-host sandbox runtime missing (need sandbox-exec or bwrap)")
            sandboxed = base
        ...
        if user and platform.system() in {"Darwin", "Linux"}:
            ...
            if platform.system() != "Darwin" and not _posix_user_exists(user):
                raise RuntimeError(f"configured codex-mcp-host user does not exist: {user}")
            if not sudo:
                raise RuntimeError("sudo is required to run codex-mcp-host under a dedicated OS user")

...
from shared.paths import agent_repo_root, base_root, gw_root, llm_root
...
async def _report_async(tail_lines: int) -> str:
    root = base_root()
    agent_repo = agent_repo_root()
    session_root = agent_repo / "sessions"
    latest_session = None
    if session_root.exists():
        session_files = [p for p in session_root.iterdir() if p.is_file() and p.name != "sessions.json"]
        if session_files:
            latest_session = sorted(session_files, key=lambda p: p.name)[-1]
    ...
    lines.append(f"agent_repo_root: {agent_repo}")
    lines.append(f"gw_root: {gw_root()}")
    lines.append(f"llm_root: {llm_root()}")
    try:
        ai = ProcClient("codex-mcp-host")
        ai.request_timeout_sec = DOCTOR_RPC_TIMEOUT_SEC
        _, runtime_health = await ai.request("codex.runtime.health", {})
        lines.append(f"codex_runtime: {runtime_health.get('result', {})}")
        await ai.close()
    except Exception as exc:  # noqa: BLE001
        lines.append(f"codex_runtime: error ({exc})")
    ...
    if latest_session is not None:
        ...
        lines.append(_tail_text(latest_session, tail_lines))
    else:
        lines.append("")
        lines.append("Latest Session")
        lines.append("--------------")
    ...
    debug_files = [
        ...
        agent_repo / "sessions" / "sessions.json",
        agent_repo / "tasks" / "task_index.json",
        agent_repo / "system" / "maintenance_state.json",
    ]
    lines.append("")
    ...
    key_logs = ["sheriff-gateway", "codex-mcp-host", "telegram-listener", "sheriff-updater"]
    lines.append("Logs")
    lines.append("----")
    for svc in key_logs:

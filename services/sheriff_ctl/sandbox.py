...
from shared.paths import agent_repo_root, gw_root, llm_root
...
def _darwin_ai_worker_launcher_path() -> Path:
    return Path("/usr/local/bin/sheriff-codex-mcp-host-launch")
...
; Allow the dedicated codex-mcp-host runtime under the service user's home.
...
def cmd_sandbox(args):
    ...
    print(f"codex-mcp-host user: {current_user or '(current user)'}")
    print(f"codex-mcp-host network: {'allowed' if _network_allowed_for_ai_worker() else 'disabled'}")
